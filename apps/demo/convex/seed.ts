  ...
  handler: async (ctx) => {
    ...
    if (existingSettings) {
      ...
      await ctx.db.insert("site_settings", {
        name: "Vex CMS Demo",
        description: "A live demo of Vex CMS — the headless CMS built for Convex.",
        metaTitle: "Vex CMS Demo",
        metaDescription:
          "Try Vex CMS live. Real-time content management powered by Convex. All changes reset daily.",
...
/**
 * Patch-based reset: update existing seeded documents to their original content,
 * delete any extra documents not in the seed set, and create missing seed docs.
 *
 * This avoids a delete+recreate cycle that would cause 404s for active users.
 * Documents are patched in-place so real-time subscribers see the update instantly.
 */
export const patchReset = internalMutation({
  args: {},
  handler: async (ctx) => {
    let patched = 0
    let created = 0
    ...
    const SEED_PAGE_SLUGS = ["home", "features", "pricing", "roadmap"]

    // --- Patch or create seeded pages ---
    // We call seedMarketingSite which already handles "skip if exists".
    // But we want to PATCH existing docs, not skip them.
    // So first, delete the content of existing seeded pages to force re-creation.
    // Actually — let's just collect the seed data and patch directly.

    // For pages: patch existing, create missing, delete extras
    const allPages = await ctx.db.query("pages").collect()
    const seededPageMap = new Map<string, any>()

    // Build the seed data inline (same as seedMarketingSite but as data objects)
    // We'll read from seedMarketingSite by running it after cleanup.
    // Simpler approach: delete non-seed pages, then let seedMarketingSite handle the rest.

    for (const page of allPages) {
      if (!SEED_PAGE_SLUGS.includes(page.slug as string)) {
        await ctx.db.delete(page._id)
        deleted++
      } else {
        // Delete seeded pages too — they'll be recreated with fresh content
        await ctx.db.delete(page._id)
        deleted++
      }
    }

    // Delete all headers, footers (will be recreated)
    const allHeaders = await ctx.db.query("headers").collect()
    for (const h of allHeaders) {
      await ctx.db.delete(h._id)
      deleted++
    }

    const allFooters = await ctx.db.query("footers").collect()
    for (const f of allFooters) {
      await ctx.db.delete(f._id)
      deleted++
    }

    // Delete extra themes (keep none — seed doesn't create themes)
    ...
    // Reset site_settings — delete and recreate
    const allSettings = await ctx.db.query("site_settings").collect()
    ...
    const allVersions = await ctx.db.query("vex_versions").collect()
    for (const v of allVersions) {
      await ctx.db.delete(v._id)
      deleted++
    }

    return { patched, created, deleted }
  },
})

/**
 * Daily reset: clean up extra content, then reseed with demo data.
 * Called by the cron job in crons.ts.
 *
 * Uses an action to orchestrate two mutations sequentially:
 * 1. Clean up — delete extras and stale content
 * 2. Reseed — recreate all seeded content fresh
 *
 * The gap between cleanup and reseed is near-instant (same action context).
 * Active users see content refresh via Convex real-time subscriptions.
 */
export const resetAndReseed = internalAction({
  args: {},
  handler: async (ctx) => {
    await ctx.runMutation(internal.seed.patchReset)
    await ctx.runMutation(internal.seed.seedMarketingSite)
    console.log("[demo] Daily reset complete — content cleaned and reseeded")
  },
})

 ...
 * is the only collection this app's route mapper resolves paths for (see
 * `vex.config.ts`'s `routes.map`), so any other slug lists nothing — the
 * mapper would return `[]` for its documents regardless, and reading them

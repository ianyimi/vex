...
import { createDocument, deleteDocument, getDocument, listDocuments, updateDocument } from "../model/api/user"

const SLUG = "user" as const
...
export const update = mutation({
  args: { id: v.id("user"), fields: v.any() },
  handler: async (ctx, args) => {
    const { roles, user } = await requireAuth(ctx)
    hasPermission({
      access: vexConfig.access,

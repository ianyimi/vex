...
import { defineCollection, defineConfig, defineGlobal, defineServerConfig, relationship } from "../index";
...
describe("generateVexTypes — header", () => {
  ...
  it("suppresses no-empty-object-type, because the empty maps are load-bearing", () => {
    ...
    const output = generateVexTypes({ config: defineServerConfig({ config: defineConfig() }) });
...
describe("generateVexTypes — globals", () => {
  ...
  it("emits type GlobalSlug = never when no globals are registered", () => {
    const config = defineServerConfig({ config: defineConfig({ globals: [] }) })
    const output = generateVexTypes({ config });
...
describe("generateVexTypes - CustomActionsBySlug", () => {
  ...
  it("emits an empty block when RBAC is not configured at all", () => {
    const config = defineServerConfig({ config: defineConfig({ collections: [users] }) })
    const output = generateVexTypes({ config });
    expect(output).toContain("CustomActionsBySlug: {\n\n}");
  });
});
...
describe("generateVexTypes - index-less collections", () => {
  it("emits {} for a collection with no indexed fields, never `never`", () => {
    ...
    const config = defineServerConfig({ config: defineConfig({ collections: [bare] }) })
    const output = generateVexTypes({ config });

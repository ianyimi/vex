...
- [ ] **The step-8 docs sweep ran, and its report names every file it touched.** Not a spot-check: the surface list comes from `git diff` on `packages/*/src/**`, and each surface is grepped for BOTH its old and new spelling across `apps/docs`, every `README.md`, and `apps/www`'s blocks + `convex/seed.ts`. A feature that moved to shipped is moved in all three roadmap surfaces (`apps/docs/roadmap.md`, the `Roadmap` block config, `convex/seed.ts`).

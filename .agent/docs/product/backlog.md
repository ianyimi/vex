...
## Consume `@vexcms/react` from source in dev (kill the tsup watch loop)

**What.** Have the apps resolve `@vexcms/react` (and the other workspace packages) to their
TypeScript source in development instead of to `dist`, so Next's own watcher drives HMR and
the `tsup --watch` rebuild step leaves the dev loop entirely.

**Why.** `tsup --watch` silently drops filesystem events for some directories, so edits to
those files never trigger a rebuild and never reach the browser — observed repeatedly on
2026-09-19 for `packages/react/src/components/livePreview/**`, where sibling directories
(`src/hooks/**`, `src/context/**`) fired normally and raw `fs.watch` plus a standalone
chokidar process both saw the events tsup missed. The mechanism is tsup's ignore predicate
`globSync(path, { ignore }).length === 0` combined with tinyglobby's `onlyFiles: true`
default, which makes every directory path read as ignorable. Scoping the watcher with
`tsup --watch src` (done) helped but did not fully fix it. Consuming source also upgrades
package edits from "rebuild the bundle, full reload" to real React Fast Refresh.

**Lift.** Medium — NOT config-only, which is the trap. `tsup.config.ts` injects
`"use client"` as a *banner* on `dist/index.js`; the source modules do not all carry the
directive themselves (`src/index.ts` opens with a bare import; `src/components/Icon.tsx` has
none). Pointing Next at `src/index.ts` therefore fails the first RSC import with "You're
importing a module that depends on useState into a React Server Component module" — the exact
failure the banner comment in `tsup.config.ts` documents. The work is: decide the real
client/server boundary per module, add `"use client"` at the source files that need it,
then `transpilePackages` + the `source` resolve condition in each app's `next.config.ts`,
verified across `apps/www`, `apps/test`, and both `create-vexcms` templates (prod builds
included, since resolution changes there too unless gated to dev).

**Why deferred.** Surfaced mid-session while implementing live preview; too broad to bolt on
without its own spec and verification pass. Workaround in the meantime: when a file stops
triggering rebuilds, touch `packages/react/src/index.ts` — any detected change rebuilds the
whole bundle, including the edits the watcher missed.

**Note.** LSP is unaffected either way: `apps/www/tsconfig.json` already sets
`customConditions: ["source"]`, so `gd`/`gr` resolve into `packages/*/src` today independently
of how Next resolves at runtime.

**Detail.** `packages/react/tsup.config.ts` (banner), `packages/*/package.json#exports.source`,
`apps/*/next.config.ts`.

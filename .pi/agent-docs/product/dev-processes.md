...
- **Convex:** `convex-test` (in-memory) for any Convex function tests
- Packages with tests: `@vexcms/core`, `@vexcms/react`, `@vexcms/cli`, `@vexcms/better-auth`, `@vexcms/file-storage-convex`, `create-vexcms`
- Agent must run `pnpm --filter <affected-pkg> test` after non-trivial changes; do not run root `pnpm test` in a loop (slow).

## Error Surfaces (Debug Hierarchy)

Full details: `.pi/agent-docs/standards/debug-hierarchy.md`.

1. **Browser devtools console + Network tab** — apps/www admin UI, most user-visible bugs surface here
2. **tmux pane 1** — `pnpm dev` output: Next.js errors, tsup build failures, Convex function errors
...
5. `pnpm release` builds and publishes `@vexcms/*` + `create-vexcms` to npm.

The `/changeset` prompt (see Phase 9) drafts changeset descriptions by reviewing commits since the last release.

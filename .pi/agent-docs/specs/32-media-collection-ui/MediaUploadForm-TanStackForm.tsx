 ...
 *
 * Uses TanStack Form for array state management (similar to FormArray/FormBlocks).
 *
 ...
 * - Stages files using TanStack Form array field API
...
export function MediaUploadForm({
  ...
  // Initialize TanStack Form with staged files
  const form = useForm<{ files: StagedFile[] }>({
    defaultValues: {
  ...
  // Subscribe to files array for reactive UI
  ...
  const addFiles = (newFiles: File[]) => {
    ...
    form.setFieldValue("files", multi ? [...files, ...staged] : staged);
  };
      ...
      <>
        <div className="p-5">
          ...
          >
            ...
            <div>
              <p className="text-sm font-medium">Drag files here, or choose</p>
                ...
                <div className="flex flex-col gap-4 pt-3">
                  ...
                  <form.Field name={`files[${index}].alt`}>
                    {(field) => (
                      <div className="space-y-1.5">
                        <Label htmlFor={field.name} className="text-xs font-medium">
                          Alt text {collection.fields.alt?.required && <span className="text-destructive">*</span>}
                        </Label>
                        <Input
                          id={field.name}
                          value={field.state.value}

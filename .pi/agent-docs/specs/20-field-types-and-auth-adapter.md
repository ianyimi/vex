...
**Status:** In progress — Part 1 partially complete (`checkbox`, `number`, `date`, `select`, `url` implemented; `password`, `json`, `relationship` remaining). Part 2 not started.  
...
1. **New field types** — implement `checkbox`, `number`, `select`, `date`, `url`, `password`, `json`, and `relationship` following the pattern in `adding-a-field-type.md`.
...
- `email` is **not** a separate field type. Use `text()` for email addresses.
- `url()` is a generic URL field — validation is URL format only via `z.url()`; no image preview or load validation. Use `text()` for email.
...
- **Convex validator:** `v.array(v.union(v.literal("opt1"), v.literal("opt2"), ...))` — typed from configured options. Wraps in `v.optional()` when not required.
- **Form value type:** `string[]`
- **Default value:** `[]`
- **Config options:**
  - `options: { label: string; value: string; badgeColor?: string }[]` — no string shorthand; `badgeColor` is an optional hex color for badge display
  - `hasMany?: boolean` — default `false` (single-select). When `true`, allows selecting multiple values simultaneously.
  - `optionInterfaceName?: string` — optional name for the generated TypeScript union type during schema generation
- **Input:** `<MultiSelect single={!fieldDef.hasMany}>` populated with the field's options; badge colors applied when configured
- **Cell:** The matching option's `label` (not raw value). Falls back to raw value if no match. Badges shown with `badgeColor` when configured.
- **Zod schema:** Array schema with typed union refinement against valid option values
...
- **Form value type:** `string`
- **Default value:** `""`
- **Input:** `<Input type="text" />` — URL format validation happens at submit time via `z.url()`, not via the browser's built-in `type="url"` validation. No image preview or load validation.
- **Cell:** `VexLink` rendering the URL value. For title fields: links to the document edit page. For non-title fields: links to the URL value itself. Values longer than 77 characters are truncated with the full URL in the `title` attribute.
- **Zod schema:** `z.url().default("")` for optional; `z.url().min(1, "This field is required.").default("")` for required. Length constraints (`min`/`max`) applied via `applyBaseInputSchemaMeta`.
...
6. `url` — URL text input with Zod `z.url()` validation
...
### Completed (synced 2026-04-22)

**`select` field** — Implemented with these deviations from spec:
- `defaultValue` is `[]` (not `""`). Select stores an array because it supports multi-select.
- `options` only accepts the object form `{ label, value, badgeColor? }` — no string shorthand. `badgeColor` is a new optional field not in the original spec.
- `hasMany: boolean` (default `false`) was added to toggle between single and multi-select. The spec had single-select only.
- `optionInterfaceName?: string` was added for TypeScript union type generation.
- Convex validator generates `v.array(v.union(v.literal("..."), ...))` typed to configured options — not the spec's `v.string()`.

**`url` field (renamed from `imageUrl`)** — Implemented as a generic URL field, not image-specific:
- Renamed from `imageUrl()` to `url()` and `ImageUrlField` to `UrlField` throughout.
- No image preview or load validation. The original spec required an `<img>` preview on blur and a blocking `onError` field error. The implementation simplifies to URL format validation only via `z.url()` at submit time.
- Input is `type="text"` (not `type="url"`) — avoids browser-native URL validation in favor of Zod-controlled validation.
- Cell renders as `VexLink` (not an `<img>` thumbnail). For title fields: edit-page link. For non-title fields: direct URL link.

### Remaining work
- `password`, `json`, `relationship` field types

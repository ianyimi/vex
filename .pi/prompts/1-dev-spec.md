...
- **Mutation client files use factory functions wrapping `useConvexMutation`, not bare re-exports**: `export function create() { return useConvexMutation(vexConvexApi.create); }`. Consumers call it as `mutationFn: create()` inside `useMutation()`. More ergonomic than re-exporting the reference, keeps the hook call at React's top level. Follow for all mutation client files. *(sync-spec 24)*
- **`GenericId.__tableName` is TypeScript-only, NOT a runtime property**: `Id<"posts">` is `string & { __tableName: "posts" }` at the type level only. At runtime, Convex IDs are plain strings. Accessing `(id as any).__tableName` always returns `undefined`. When you need the table name from an ID at runtime (e.g., `get` with `depth`), use `(id as string).split(";").at(-1)` (the format `convex-test` uses: `"{random};{tableName}"`) as a fallback. In production Convex, IDs have no semicolon, so this gracefully returns `undefined` and you can degrade safely. *(sync-spec 24)*
- **`isMounted` guard on cells that read Convex subscription data**: Cells that render Convex subscription data via `convexQuery` + `useQuery` must return a consistent SSR placeholder until after mount. Pattern: `useState(false)` + `useEffect(() => setIsMounted(true), [])` — render `—` when `!isMounted`. Without this, Convex's reactive model pushes subscription data into TanStack Query's cache via `useSyncExternalStore` during the hydration render, causing a systematic mismatch on every page load. Apply to all list-view cells displaying relationship, computed, or subscription-driven content. *(sync-spec 24)*
